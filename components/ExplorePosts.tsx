...
import { Avatar, Grid, Paper, Typography, Button, SwipeableDrawer, Input, TextField } from "@mui/material";
...
function ExplorePosts() {
  ...
  const { data: Posts, isLoading,isFetched } = useQuery({
    ...
    queryFn: async (): Promise<UserPosts[]> => {
      return axios.post("/api/ExplorePosts",{userId:user?.id})
      .then((res) => {
        console.log("ExplorePosts", res.data);
        if(res.data.error){
    ...
    enabled:!!user //only run query if the user data is received successfully
  });
  ...
  const likePostMutation = useMutation(
    ...
    {//`On mutate is for the client side changes - to increase or decrease like count
      onMutate: async ({ postId }) => { //`the post if is extracted from variables object
        ...
        queryClient.setQueryData(["ExplorePosts"], (data: any) => { //`Data is the array of posts
          if (!data) return data;
  ...
  return (
    <main className="my-3 p-2">
      ...
      // if posts are fetched but they are undefined or length is zero then
      :isFetched&&(!Posts || Posts?.length == 0 )? (


        <section className="h-96 w-ful flex justify-center items-center ">
